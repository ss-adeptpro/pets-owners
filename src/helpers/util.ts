...
export const groupListByKey = <T>(keys: (keyof T)[]) => (array: T[]): Record<string, T[]> =>
  array.reduce((objectsByKeyValue, obj) => {    
    ...
    //sort the cats
    petsArray && petsArray.length > 0 && petsArray.sort((a:TPet, b:TPet) => b.name.localeCompare(a.name))
    const objTemp = { ...obj, pets: petsArray };
    
    //exclude owners having no pets
    petsArray && petsArray.length > 0 && (objectsByKeyValue[value.toLowerCase()] = (objectsByKeyValue[value.toLowerCase()] || []).concat(objTemp));
